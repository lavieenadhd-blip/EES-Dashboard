...
# 1. LIVE GLOBAL DATA INGESTION
@st.cache_data(ttl=86400, show_spinner=False) # Caches the data for 24 hours so the site loads instantly for users
def fetch_global_ees_data():
    ...
    recent_df["M"] = np.clip((recent_df["M_proxy"] - M_min) / (M_max - M_min), 0.20, 0.95)
...
# 2. EES ENGINE CALCULATION
st.sidebar.header("⚙️ Engine Tuning")
kappa_val = st.sidebar.selectbox("Safety Penalty Exponent (κ)", [1, 2, 3], index=1)
